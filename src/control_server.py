...
/* Tab bar */
.tab-bar { display: flex; gap: 0; background: var(--panel); border-bottom: 1px solid var(--border); position: sticky; top: 49px; z-index: 9; overflow-x: auto; }
.tab-bar button { padding: 11px 18px; border: none; border-bottom: 3px solid transparent; background: none; color: var(--muted); font-family: inherit; font-size: 13px; font-weight: 600; cursor: pointer; letter-spacing: 0.4px; white-space: nowrap; transition: color 0.15s; }
.tab-bar button:hover { color: var(--text); }
.tab-bar button.active { color: var(--tab-active); border-bottom-color: var(--tab-active); }

/* Tab panels */
.tab-panel { display: none; max-width: 960px; margin: 0 auto; padding: 18px; }
.tab-panel.active { display: block; }
.tab-panel .grid { display: grid; grid-template-columns: 1fr 1fr; gap: 14px; }
...
.fluid-grid { display: grid; grid-template-columns: repeat(2, 1fr); gap: 8px; }
...
<div class="tab-panel" id="tab-effects" role="tabpanel" aria-labelledby="tabbtn-effects">
  <div class="section">
    ...
    <h2 style="margin-top:16px">Effect Amounts</h2>
...
<div class="tab-panel" id="tab-fluid" role="tabpanel" aria-labelledby="tabbtn-fluid">
  <div class="section">
    ...
    <div class="effects-grid" style="grid-template-columns: 1fr; margin-bottom:16px">
      <div class="toggle-btn" data-ns-toggle="effect_fluid_enabled">GPU Fluid</div>
    </div>
    <div class="fluid-grid">
      <div class="row" style="margin-bottom:0">
        <label>Intensity %</label>
        <input type="range" id="s-fluid_intensity" min="0" max="200" step="5" data-ns="fluid_intensity">
        <span class="val" id="v-fluid_intensity"></span>
      </div>
      <div class="row" style="margin-bottom:0">
        <label>Curl (vorticity)</label>
        <input type="range" id="s-fluid_curl" min="0" max="100" step="5" data-ns="fluid_curl">
        <span class="val" id="v-fluid_curl"></span>
      </div>
      <div class="row" style="margin-bottom:0">
        <label>Density Dissipation</label>
        <input type="range" id="s-fluid_density_dissipation" min="1" max="100" step="1" data-ns="fluid_density_dissipation">
        <span class="val" id="v-fluid_density_dissipation"></span>
      </div>
      <div class="row" style="margin-bottom:0">
        <label>Velocity Dissipation</label>
        <input type="range" id="s-fluid_velocity_dissipation" min="1" max="40" step="1" data-ns="fluid_velocity_dissipation">
        <span class="val" id="v-fluid_velocity_dissipation"></span>
      </div>
      <div class="row" style="margin-bottom:0">
        <label>Pressure %</label>
        <input type="range" id="s-fluid_pressure" min="0" max="100" step="5" data-ns="fluid_pressure">
        <span class="val" id="v-fluid_pressure"></span>
      </div>
    </div>
  </div>
</div>
...
<div class="tab-panel" id="tab-themes" role="tabpanel" aria-labelledby="tabbtn-themes">
  <div class="section">
    ...
    <div class="theme-grid" id="theme-btns">
      <span class="no-themes">No user themes found.</span>
    </div>
  </div>
</div>
...
// Effect toggles (including fluid enable)
document.querySelectorAll('[data-ns-toggle]').forEach(function(el){
...
function loadThemes(){
  fetch('/api/themes').then(function(r){ return r.json(); }).then(function(d){
    var cont=document.getElementById('theme-btns');
    if(!d.themes||!d.themes.length){
      cont.innerHTML='<span class="no-themes">No user themes found.</span>';
      return;
    }
    cont.innerHTML='';
