...
class ThemeSettingsScreen:
    ...
    def __init__(self, screen: pygame.Surface) -> None:
        ...
        self._active_index: int   = themes_mod.get_active_bank_index()
    ...
    def _handle_click(self, pos: tuple[int, int]) -> str | None:
        ...
        if self._editing_index >= 0:
            ...
            if idx >= len(self._row_name_rects) or not self._row_name_rects[idx].collidepoint(pos):
                self._commit_edit()  # no rename needed
    ...
    def _save_as_new(self) -> None:
        ...
        bank = themes_mod.snapshot_bank(name)
        self._banks.append(bank)
        self._active_index = len(self._banks) - 1
        themes_mod.save_banks(self._banks)
        themes_mod.set_active_bank_index(self._active_index)
        self._build_layout()
    ...
    def _update_active(self) -> None:
        ...
        if idx >= len(self._banks):
            idx = len(self._banks) - 1
        name = self._banks[idx].get("name", f"Bank {idx + 1}")
        updated = themes_mod.snapshot_bank(name)
        self._banks[idx] = updated
        themes_mod.save_banks(self._banks)
        themes_mod.set_active_bank_index(idx)

    def _load_bank(self, index: int) -> None:
        if 0 <= index < len(self._banks):
            self._active_index = index
            themes_mod.set_active_bank_index(index)
            themes_mod.apply_bank_to_config(self._banks[index])

    def _delete_bank(self, index: int) -> None:
        if 0 <= index < len(self._banks):
            self._banks.pop(index)
            if self._active_index >= len(self._banks):
                self._active_index = max(0, len(self._banks) - 1)
            themes_mod.save_banks(self._banks)
            themes_mod.set_active_bank_index(self._active_index)
            self._build_layout()

    def _do_scroll(self, delta: int) -> None:
        total_h   = len(self._banks) * (ROW_H + ROW_GAP)
    ...
    def _build_layout(self) -> None:
        ...
        title_surf = self._title_font.render("Bank Manager", True, TITLE_COLOR)
        ...
        for i in range(len(self._banks)):
    ...
    def _draw_top_buttons(self) -> None:
        self._draw_btn(
            self._save_rect, "SAVE AS NEW BANK",
            self._hover_save,
            SAVE_BG, SAVE_BG_HOVER, SAVE_TEXT, SAVE_TEXT_HOVER,
        )
        self._draw_btn(
            self._update_rect, "UPDATE ACTIVE BANK",
            self._hover_update,
    ...
    def _draw_rows(self) -> None:
        ...
        if not self._banks:
            msg = self._row_font.render(
                "No banks saved yet.  Press  SAVE AS NEW BANK  to create one.",
                True, (130, 130, 140),
            )
        ...
        for i, bank in enumerate(self._banks):
            ...
            display_text = (self._editing_text + "|") if is_editing else bank.get("name", f"Bank {i+1}")
            ...
            # ── Color swatch (note color ch1 | interior color ch1) ──
            _ch1 = bank.get("channels", {}).get("1", {})
            note_r = _ch1.get("note_color_r", 0)
            note_g = _ch1.get("note_color_g", 230)
            note_b = _ch1.get("note_color_b", 230)
            led_r  = _ch1.get("note_interior_r",  120)
            led_g  = _ch1.get("note_interior_g",  255)
            led_b  = _ch1.get("note_interior_b",  255)
